...
```
ブラウザで `http://localhost:8768` を開く（localhostはHTTPS扱いなので音声入力もSWも動く）。

## 同じWiFiのスマホから一時的に試す（簡易・非推奨）
PCで上のサーバーを起動し、スマホで `http://<PCのIPアドレス>:8768` を開く。
...
## 窓（リフレーミング）と AI 設定 — 2026-07-02 追加
- 「問い」のあとに任意ステップ**「窓」**が増えました。出来事を別の見方（友人の窓・時間の窓・反証の窓など）から眺めるカードを2〜3枚提示し、1枚選ぶと**不安度をもう一度測って**記録します（水位グラフは測り直した値を使う）。
- 窓は助言をしません。命令形・「すべき」を使わず、「〜かもしれない」という可能性の提示だけ。
- **⚙（設定）で Claude APIキー**（console.anthropic.com で発行）を入れると、鏡の仕分けと窓が書いた内容に合わせたAI生成になります。**キーは端末のlocalStorageにのみ保存**（GitHub Pages やサーバーには送られません）。未設定でも簡易仕分け＋定型の窓で全機能が使えます。
- 文章に希死念慮などの兆候がある場合、窓は視点変えをせず**相談窓口**（#いのちSOS ほか）を案内します。⚙画面にも常設。
